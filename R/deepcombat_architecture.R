...
deepcombat_cvae <- nn_module(
  ...
  initialize = function(feature_dim, latent_dim, 
                        n_hidden = 3, n_batch, n_covariate) {
  ...
  harmonize = function(setup_obj,
    ...
    combat_batch_list <- convert_batch_for_combat(batch, target_batch)
    ...
    if (correct[1] == "combat") {
      corrected_mu <- torch_tensor(t(neuroCombat(dat = t(as.matrix(feat_mu)),
    ...
    if (correct[2] == "combat") {
      ...
      corrected_resids <- torch_tensor(t(neuroCombat(dat = t(as.matrix(feat_resids)),
                                      batch = combat_batch_list$batch,
                                      mod = as.matrix(torch_ds$covariates),
                                      ref.batch = reference_batch,
                                      verbose = verbose)$dat.combat))
    ...
    return(list(harmonized = as.matrix(feat_combat_restyled + corrected_resids),
                latent_logvar = as.matrix(feat_logvar),
                additional_outputs = list(unharmonized_latent_representation = as.matrix(feat_mu),
